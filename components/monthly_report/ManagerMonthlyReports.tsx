...
import {fetchManagerUsersAndRequests } from "@/lib/dashboard/action";
...
export default function ManagerMonthlyReports({ managerID }: ManagerReportsProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { users, requests } = await fetchManagerUsersAndRequests(managerID);
        setUsers(users);
  ...
  const exportToPDF = () => {
    ...
    doc.text(`Manager Reimbursements Report`, 105, 20, { align: 'center' });
    ...
    // For average processing time, you might need to adjust based on your actual data structure
    const avgProcessingTime = "1.8"; // This should be calculated from your data
    ...
    const metricsData = [
      ...
      ['Total Budget', ` ${totalAllocatedBudget.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`]
    ];
    ...
    // If no monthly data (perhaps filtered to a specific month), show current month data
    if (monthlyTableData.length === 0) {
      ...
      const total = currentMonthRequests.reduce((sum, r) => sum + parseFloat(r.amount ?? "0"), 0);
      const approved = currentMonthRequests
        .filter(r => r.status === "Approved")
        .reduce((sum, r) => sum + parseFloat(r.amount ?? "0"), 0);
      const pending = currentMonthRequests
        .filter(r => r.status === "Pending")
        .reduce((sum, r) => sum + parseFloat(r.amount ?? "0"), 0);
      
      monthlyTableData.push([
        months[currentMonth + 1].label,
    ...
    // Add Employee Expense Details section
    doc.addPage();
    doc.setFontSize(14);
    doc.text('Employee Expense Details', 14, 20);
    ...
    autoTable(doc, {
      startY: 30,
      head: [['Employee', 'Submissions', 'Total', 'Approved', 'Pending']],
      body: employeeData,
      theme: 'grid',
      headStyles: { 
        fillColor: [59, 130, 246],
    ...
    doc.save(`reimbursement-report-${new Date().toISOString().slice(0, 10)}.pdf`);
  };
  ...
  const exportToExcel = () => {
    ...
    const employeeData = users.map(user => {
      ...
      return {
        ...
        'Total Submissions': userRequests.length,
        'Total Amount': total,
    ...
    utils.book_append_sheet(wb, employeeSheet, 'Employee Expenses');
    utils.book_append_sheet(wb, summarySheet, 'Summary');
  
    // Export the workbook
    writeFile(wb, `reimbursement-report-${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
  return (
  ...
  <div>
    ...
    <p className="text-sm text-gray-600">
      Detailed month-wise insights into company expenses and reimbursements
    </p>
  </div>
  ...
  <div className="flex items-center gap-2">
    ...
    <Button onClick={exportToExcel} variant="outline">
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Card className="flex-1 min-w-[250px]">
          <CardHeader>
            <CardTitle>Approved Request and Amount</CardTitle>
            <CardDescription>
              Approvals in {selectedMonth !== "0"
      ...
      <div className="flex gap-6 mt-6">
        ...
        <Card className="flex-1 h-[340px]">
          <CardHeader className="pb-2">
            <CardTitle className="text-base">Monthly Expense Trends</CardTitle>
            <CardDescription className="text-sm">
      ...
      <Card className="h-[340px]">
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-sm">
          Weekly average and totals for 2025
          </CardDescription>
        </CardHeader>
          ...
          {users.length > 0 ? (
            <div className="flex flex-col gap-6">
              {users.map((user) => {
                ...
                return (
                  <Card key={user.id} className="w-full p-6 shadow-sm border rounded-lg h-[120px] flex justify-between items-center">
